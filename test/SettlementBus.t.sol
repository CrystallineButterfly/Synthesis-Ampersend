...
contract SettlementBusTest is Test {
    SettlementBus private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_ampersend");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new SettlementBus(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        SettlementBus freshController = new SettlementBus(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
