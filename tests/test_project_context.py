...
MODULE = importlib.import_module('agents.ampersend_bus')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'ampersend_bus')
        self.assertEqual(spec.primary_contract_name, 'SettlementBus')
